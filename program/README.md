...
| R2 | **Classifier accuracy ceiling** — capped by label ambiguity, not model horsepower. At `v3.0.0` (n=54 gold): category **92.6%** / macro-F1 0.911, operational-domain **92.6%** / macro-F1 0.933, region **87.0%** / macro-F1 0.927. The ceiling claim has held across three versions, and two measured escalations were *declined* on the strength of it — BM25 grounding (`classifier/ADR-012`) and tiered routing (`classifier/ADR-013`), the latter at ~1.97x cost for +0 rows. The region axis added a second ceiling mechanism: the misses are one cluster (gold `global` pulled to a specific region), i.e. the no-guessing rule, not label overlap. *(Numbers restated 2026-07-19 from v2's 88.9%; this row had been two versions stale. Source of truth is the classifier's generated `evals/metrics.json` — do not retype these by hand.)* | Medium | Don't escalate the model (per `system/SYS-002`); refine taxonomy or use an LLM judge on boundary cases; set the expectation in product metrics | `classifier/ADR-001`, `system/SYS-002` |
