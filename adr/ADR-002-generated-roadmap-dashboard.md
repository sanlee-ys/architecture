...
same problem [`ADR-001`](ADR-001-documentation-portal.md) solved for documentation, not yet
solved for status.

`ADR-001`'s Phase 2 (the interactive system map) answers *how the pieces connect*. This is a
...
already the curated, current source of truth. That's exactly the anti-pattern `ADR-001` named and
...
- **The dashboard cannot drift from reality** — it has no state of its own to drift. A stale
  ...
  place to fix it (same guarantee `ADR-001` gives for docs generally).
...
- **Doesn't touch `ADR-001` Phase 2** (the interactive topology map). That's still open and answers
...
|--------|-------------------|
| **A `roadmap.yaml` per app repo, hand-curated** | Duplicates `program/README.md`'s Now/Next/Later section, which is already the curated source of truth. Adds a new file to remember to update in three repos for information that already exists in one. The exact anti-pattern `ADR-001` rejected for docs. |
...
| **Fetch version + roadmap over the GitHub API at build time (no local clone)** | The portal's CI already clones all three app repos as siblings for the docs aggregation step (`ADR-001`); reading local files is simpler and avoids a second network-dependent code path for the same data. |
