# ADR-001: A generated documentation portal — one browsable view over the whole system

**Status:** Accepted
**Date:** 2026-06-23 (re-tiered from `SYS-008` on 2026-07-18)
**Deciders:** San Lee

> **Re-tiered from `system/SYS-008`, 2026-07-18.** The portal is a build feature of *this*
> repo. It **reads** six repos; it does not bind them, and this ADR's own text argues that as
> a feature. Under [`SYS-001`](../decisions/SYS-001-record-architecture-decisions.md)'s
> promotion bar it fails prong 1 — crossing repo boundaries — so it belongs in the local tier.
> It only ever held a system number because this repo had no `adr/` namespace until today,
> which is the mechanism the 2026-07-18 audit identified: the log was **under-tiered, not
> padded**.
>
> `SYS-008` remains as a tombstone at `decisions/SYS-008-documentation-portal.md`; the number
> is retired, never reused, and existing citations still resolve.
>
> **One constraint was deliberately NOT moved down with this ADR.** `scripts/build_portal.py`
> imposes a real cross-repo layout expectation — every app repo must keep `README.md`,
> `docs/` and `decisions/` at its root under exactly those names, or the portal silently drops
> that content with no error. That binding is genuinely system-level and now lives in
> [`engineering/README.md`](../engineering/README.md)'s house standards. Filing it here would
> have made the log tidier and the system less safe.
...
quietly preempt the actual keystone work ([`SYS-017`](../decisions/SYS-017-evals-as-ci.md) evals-as-CI; the
`notes-api` classify-and-writeback loop).
